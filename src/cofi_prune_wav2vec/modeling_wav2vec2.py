...
def _sample_negative_indices(
    ...
    mask_time_indices = (
        mask_time_indices.astype(bool) if mask_time_indices is not None else np.ones(
            features_shape, dtype=bool)
    )
...
class Wav2Vec2Encoder(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = 1.0 - \
                attention_mask[:, None, None, :].to(dtype=hidden_states.dtype)
...
class Wav2Vec2PreTrainedModel(PreTrainedModel):
    ...
    def load_adapter(self, target_lang: str, force_load=True, **kwargs):
        ...
        state_dict = {k: v.to(adapter_weights[k])
...
class TDNNLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        weight = self.kernel.weight.view(
            self.out_conv_dim, self.kernel_size, self.in_conv_dim).transpose(1, 2)
