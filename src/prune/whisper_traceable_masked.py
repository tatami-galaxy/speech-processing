...
from transformers.utils import ModelOutput, ExplicitEnum
...
class WhisperAttention(nn.Module):
    ...
    def forward(
        ...
        # hidden_states -> b x seq_len x embed_dim

        # get query proj
        # key matrix after X @ Q
        # shape -> b x seq_len x embed_dim
...
class WhisperDecoder(MaskedWhisperPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            # add LayerDrop (see https://arxiv.org/abs/1909.11556 for description)
            #if decoder_layer.self_attn.skip_layer:
                #continue
            if output_hidden_states:
