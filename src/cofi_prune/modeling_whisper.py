...
class SparseWhisperAttention(nn.Module):
    ...
    def forward(
        ...
        de_self_head_z_l = None,  # 1, 12, 1, 1
        de_self_mha_z_l = None,  # scalar
        de_cross_head_z_l = None,  # 1, 12, 1, 1
        de_cross_mha_z_l = None,  # scalar
        ...
        # bsz is batch size, tgt_len -> sequence length
        ...
        # decoder self attn head mask
        elif de_self_head_z_l is not None:
            attn_output = attn_output.mul(de_self_head_z_l)
        # decoder cross attn head mask
        elif de_cross_head_z_l is not None:
            attn_output = attn_output.mul(de_cross_head_z_l)
        ...
        # decoder self mha mask
        elif de_self_mha_z_l is not None:
            attn_output = attn_output.mul(de_self_mha_z_l)
        # decoder cross mha mask
        elif de_cross_mha_z_l is not None:
            attn_output = attn_output.mul(de_cross_mha_z_l)
...
class SparseWhisperEncoderLayer(nn.Module):
    def __init__(self, config: SparseWhisperConfig):
        ...
        self.self_attn = SparseWhisperAttention(
...
class SparseWhisperDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            en_head_z_l=None,
            en_mha_z_l=None,
            de_self_head_z_l=de_self_head_z_l,
            de_self_mha_z_l=de_self_mha_z_l,
            de_cross_head_z_l=None,
            de_cross_mha_z_l=None,
        )
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states, cross_attn_weights, cross_attn_present_key_value = self.encoder_attn(
                ...
                de_self_head_z_l=None,
                de_self_mha_z_l=None,
                de_cross_head_z_l=de_cross_head_z_l,
                de_cross_mha_z_l=de_cross_mha_z_l,
            )
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + hidden_states
...
class SparseWhisperPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (SparseWhisperDecoder, SparseWhisperEncoder)):
...
class SparseWhisperEncoder(SparseWhisperPreTrainedModel):
    """
...
class SparseWhisperDecoder(SparseWhisperPreTrainedModel):
    """
...
@add_start_docstrings(
    "Sparse Whisper Model outputting raw hidden-states without any specific head on top.",
    WHISPER_START_DOCSTRING,
)
class SparseWhisperModel(SparseWhisperPreTrainedModel):
    def __init__(self, config: SparseWhisperConfig):
        super().__init__(config)

        self.encoder = SparseWhisperEncoder(config)
        self.decoder = SparseWhisperDecoder(config)
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(
            ...
            # cofi decoder args
            hidden_z=hidden_z,
            de_self_head_z=de_self_head_z,
            de_self_mha_z=de_self_mha_z,
            de_cross_head_z=de_cross_head_z,
            de_cross_mha_z=de_cross_mha_z,
...
@add_start_docstrings(
    "Sparse Whisper Model with a language modeling head. Can be used for automatic speech recognition.",
    WHISPER_START_DOCSTRING,
)
class SparseWhisperForConditionalGeneration(SparseWhisperPreTrainedModel):
    base_model_prefix = "model"
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        # cofi conditional gen model args
        hidden_z = None,
        # encoder
        en_head_z = None,
        en_mha_z = None,
